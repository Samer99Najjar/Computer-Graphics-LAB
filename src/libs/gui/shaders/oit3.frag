...
vec3 diffuse( )
{
  vec3 s = normalize( LightPos.xyz - FragPos );
  vec3 n = normalize(Normal);
  return
    LightIntensity * ( objectColor.rgb +
        objectColor.rgb * max( dot(s, n), 0.0 ) );
}
...
void pass1()
{
  ...
  if( nodeIdx < max ) {
    ...
    //nodes[nodeIdx].color = vec4(objectColor, alpha);
    nodes[nodeIdx].color = vec4(computeBasicShading() * objectColor, alpha);
...
void pass2()
{
  ...
  while( n != 0xffffffff && count < maxfrags) {
    frags[count] = nodes[n];
    n = frags[count].next;
    count++;
  }
...
*/
  ...
  // channel.
  vec4 color = vec4(0.8, 0.8, 0.8, 0.0); // background color
