...
void crl::gui::Fancy3DApp::drawImGui()
{
	...
	ImGui::SliderFloat("transparency level", &transparency, 0.0f, 1.0f, "%.5f");
	ImGui::SliderInt("num passes", &NUM_PASSES, 1, 20);
	ImGui::SliderInt("nodes per fragment", &maxfrags, 1, 30);
	...
	ImGuizmo::ViewManipulate(glm::value_ptr(camera.viewMatrix), 10.0,
...
void crl::gui::Fancy3DApp::renderPass()
{
	...
	basicShader.setFloat("transparency",transparency);
	shader_setup(frontpeelshader);
	frontpeelshader.setVec3("lightPos", camera.position());
	frontpeelshader.setFloat("transparency", transparency);
	shader_setup(initshader);




	oitshader.setMat4("lightProjection", light.getOrthoProjectionMatrix());
	oitshader.setMat4("lightView", light.getViewMatrix());
	oitshader.setMat4("projection", camera.getProjectionMatrix());
	oitshader.setMat4("view", camera.getViewMatrix());
	oitshader.setuInt("MaxNodes", 20 * width * height);
	shader_setup(oitshader);
	oitshader.use();
	

	
	//shader_setup(dualPeelShader);
	//drawObjectsWithShadows();
	//drawObjectsWithoutShadows();
	linkedlist_main();
	//drawObjectsWithoutShadowsdual();
	//drawObjectsWithShadows();
